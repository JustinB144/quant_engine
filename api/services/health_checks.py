"""Health check functions — standalone checks for each health domain.

Extracted from HealthService (SPEC_AUDIT_FIX_04 T5) for independent testability.
Each function takes required data as parameters (no self reference) and returns
a HealthCheckResult.
"""
...
def load_trades_csv() -> Optional[pd.DataFrame]:
...
def load_benchmark_returns() -> Optional[np.ndarray]:
...
def check_signal_decay() -> HealthCheckResult:
...
def check_prediction_distribution() -> HealthCheckResult:
...
def check_information_ratio() -> HealthCheckResult:
    ...
    try:
        ...
        if len(actual) < 20:
            return _unavailable(name, domain, f"Only {len(actual)} trade returns — need 20+")
        ...
        if window < 10:
            return _unavailable(name, domain, f"Window {window} too small for IR calculation")
...
def check_survivorship_bias() -> HealthCheckResult:
...
def check_data_quality_anomalies() -> HealthCheckResult:
    ...
    try:
        ...
        issue_count = 0
        total_checked = 0
        stale_cutoff = datetime.now().timestamp() - 30 * 86400
...
def check_market_microstructure() -> HealthCheckResult:
...
def check_wrds_status() -> HealthCheckResult:
...
def check_tail_risk() -> HealthCheckResult:
...
def check_correlation_regime() -> HealthCheckResult:
...
def check_capital_utilization() -> HealthCheckResult:
...
def check_execution_quality() -> HealthCheckResult:
...
def check_signal_profitability() -> HealthCheckResult:
    """Check if long signals are actually profitable."""
    domain = "execution_quality"
...
def check_cv_gap_trend() -> HealthCheckResult:
...
def check_retraining_effectiveness() -> HealthCheckResult:
...
def check_feature_importance_drift() -> HealthCheckResult:
    ...
    try:
        ...
        try:
            ...
            history_path = Path(_RESULTS_DIR) / "feature_stability_history.json"
                ...
                if len(cycles) >= 2:
                    ...
                    if current and previous:
                        ...
                        for feat in set(current + previous):
                            ...
                            changes.append((feat, prev_rank - cur_rank))
...
def check_ensemble_disagreement(
    ...
    try:
        history = get_disagreement_history_fn(limit=ENSEMBLE_DISAGREEMENT_LOOKBACK)
        disagreement_values = [
        ...
        return _check_ensemble_disagreement_static_fallback(
...
def _check_ensemble_disagreement_static_fallback(
    ...
    if not holdout_corrs:
        return _unavailable(name, domain, "No holdout correlations or tracking history")
...
def check_ic_tracking(
    ...
    try:
        ic_history = get_ic_history_fn(limit=IC_TRACKING_LOOKBACK)
        ic_values = [h["ic_mean"] for h in ic_history if h["ic_mean"] is not None]

        if not ic_values:
            ic_from_report = _extract_ic_from_cycle_report()
...
def check_cost_surprise(
    get_execution_quality_history_fn,
) -> HealthCheckResult:
    ...
    try:
        history = get_execution_quality_history_fn(limit=EXEC_QUALITY_LOOKBACK)
        surprises = [
            h["cost_surprise_bps"]
            for h in history
...
def check_regime_transition_health() -> HealthCheckResult:
    """Check regime model quality via entropy, transition frequency, and predictive power."""
    domain = "model_governance"
