...
Subsystem metadata and dependencies are in [SUBSYSTEM_MAP.json](/Users/justinblaise/Documents/quant_engine/docs/audit/data/SUBSYSTEM_MAP.json) and [SUBSYSTEM_AUDIT_MANIFEST.md](/Users/justinblaise/Documents/quant_engine/docs/audit/data/SUBSYSTEM_AUDIT_MANIFEST.md).
...
### Configuration
- Boundary contracts: `features_to_indicators_8`, `models_to_features_6`, `features_to_data_14`, `features_to_regime_15`, `features_to_config_13` in [INTERFACE_CONTRACTS.yaml](/Users/justinblaise/Documents/quant_engine/docs/audit/data/INTERFACE_CONTRACTS.yaml).
- Hotspots: [HOTSPOT_LIST.md](/Users/justinblaise/Documents/quant_engine/docs/audit/data/HOTSPOT_LIST.md).
...
**Verify:**
```bash
jq -r '.subsystems.feature_engineering.files[]' docs/audit/data/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.target_module=="features" or .source_module=="features") | [.source_file,.target_file,.symbols_imported] | @tsv' docs/audit/data/DEPENDENCY_EDGES.json | head -n 100
```
...
### Verification steps
```bash
jq -r '.subsystems.feature_engineering.files[]' docs/audit/data/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.source_module=="features" or .target_module=="features" or .target_module=="indicators") | .import_type' docs/audit/data/DEPENDENCY_EDGES.json | sort | uniq -c
```
