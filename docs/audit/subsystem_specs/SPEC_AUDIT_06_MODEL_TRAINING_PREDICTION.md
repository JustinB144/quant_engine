...
Subsystem details are defined in [SUBSYSTEM_MAP.json](/Users/justinblaise/Documents/quant_engine/docs/audit/data/SUBSYSTEM_MAP.json) and [SUBSYSTEM_AUDIT_MANIFEST.md](/Users/justinblaise/Documents/quant_engine/docs/audit/data/SUBSYSTEM_AUDIT_MANIFEST.md).
...
**Verify:**
```bash
jq -r '.subsystems.model_training_prediction.files[]' docs/audit/data/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.source_module=="models" or .target_module=="models") | [.source_file,.target_file,.import_type] | @tsv' docs/audit/data/DEPENDENCY_EDGES.json | head -n 120
```
...
### Verification steps
```bash
jq -r '.subsystems.model_training_prediction.files[]' docs/audit/data/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.source_module=="models" or .target_module=="models") | .import_type' docs/audit/data/DEPENDENCY_EDGES.json | sort | uniq -c
```
