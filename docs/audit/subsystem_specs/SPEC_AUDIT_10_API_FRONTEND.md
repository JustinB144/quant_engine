...
Subsystem boundary and dependency data are in [SUBSYSTEM_MAP.json](/Users/justinblaise/Documents/quant_engine/docs/audit/data/SUBSYSTEM_MAP.json), [DEPENDENCY_MATRIX.md](/Users/justinblaise/Documents/quant_engine/docs/audit/data/DEPENDENCY_MATRIX.md), and [SYSTEM_ARCHITECTURE_AND_FLOWS.md](/Users/justinblaise/Documents/quant_engine/docs/architecture/SYSTEM_ARCHITECTURE_AND_FLOWS.md).
...
**Verify:**
```bash
jq -r '.subsystems.api_frontend.files[]' docs/audit/data/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.source_module=="api" or .target_module=="api") | [.source_file,.target_file,.import_type] | @tsv' docs/audit/data/DEPENDENCY_EDGES.json | head -n 180
```
...
### Verification steps
```bash
jq -r '.subsystems.api_frontend.files[]' docs/audit/data/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.source_module=="api" or .target_module=="api") | .import_type' docs/audit/data/DEPENDENCY_EDGES.json | sort | uniq -c
```
