...
**Critical**: Many imports are **lazy/conditional** (inside function bodies, behind try/except, or behind if-guards). These MUST be captured separately because they represent optional coupling that can silently break. After locating all lazy/conditionals, replace them.
