...
### Must-haves
- Review every line in all subsystem files (`19` files, `9,106` lines).
...
Subsystem metadata is defined in [SUBSYSTEM_MAP.json](/Users/justinblaise/Documents/quant_engine/docs/architecture/SUBSYSTEM_MAP.json) and [SUBSYSTEM_AUDIT_MANIFEST.md](/Users/justinblaise/Documents/quant_engine/docs/architecture/SUBSYSTEM_AUDIT_MANIFEST.md).
...
- Boundary records: `data_to_validation_10`, `data_to_config_12`, `features_to_data_14`, `data_to_kalshi_32`, `api_to_data_37` in [INTERFACE_CONTRACTS.yaml](/Users/justinblaise/Documents/quant_engine/docs/audit/INTERFACE_CONTRACTS.yaml).
...
**Verify:**
```bash
jq -r '.subsystems.data_ingestion_quality.files[]' docs/architecture/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.source_module=="data" or .target_module=="data") | [.source_file,.target_file,.import_type] | @tsv' docs/audit/DEPENDENCY_EDGES.json | head -n 80
```
...
**Verify:**
```bash
# Manual gate: 9106/9106 lines reviewed; all high-risk boundaries dispositioned
```

## Validation

### Acceptance criteria
1. 100% of `9,106` lines are reviewed and logged.
...
### Verification steps
```bash
jq -r '.subsystems.data_ingestion_quality.files[]' docs/architecture/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.target_module=="data" or .source_module=="data") | .import_type' docs/audit/DEPENDENCY_EDGES.json | sort | uniq -c
```
