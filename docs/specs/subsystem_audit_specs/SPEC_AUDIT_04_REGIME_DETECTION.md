...
Subsystem definition and dependency edges are in [SUBSYSTEM_MAP.json](/Users/justinblaise/Documents/quant_engine/docs/architecture/SUBSYSTEM_MAP.json) and [SUBSYSTEM_AUDIT_MANIFEST.md](/Users/justinblaise/Documents/quant_engine/docs/architecture/SUBSYSTEM_AUDIT_MANIFEST.md).
...
- High-risk boundaries: `backtest_to_regime_shock_1`, `backtest_to_regime_uncertainty_2`, `risk_to_regime_22`, `autopilot_to_regime_29`, `features_to_regime_15` in [INTERFACE_CONTRACTS.yaml](/Users/justinblaise/Documents/quant_engine/docs/audit/INTERFACE_CONTRACTS.yaml).
...
**Verify:**
```bash
jq -r '.subsystems.regime_detection.files[]' docs/architecture/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.source_module=="regime" or .target_module=="regime") | [.source_file,.target_file,.import_type] | @tsv' docs/audit/DEPENDENCY_EDGES.json | head -n 100
```
...
### Verification steps
```bash
jq -r '.subsystems.regime_detection.files[]' docs/architecture/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.source_module=="regime" or .target_module=="regime") | .import_type' docs/audit/DEPENDENCY_EDGES.json | sort | uniq -c
```
