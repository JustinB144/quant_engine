...
Subsystem details and dependency order are defined in [SUBSYSTEM_AUDIT_MANIFEST.md](/Users/justinblaise/Documents/quant_engine/docs/architecture/SUBSYSTEM_AUDIT_MANIFEST.md).
...
**Verify:**
```bash
jq -r '.subsystems.evaluation_diagnostics.files[]' docs/architecture/SUBSYSTEM_MAP.json | xargs wc -l
```
...
**Verify:**
```bash
jq -r '.edges[] | select(.source_module=="evaluation" or .target_module=="evaluation") | [.source_file,.target_file,.import_type] | @tsv' docs/audit/DEPENDENCY_EDGES.json
```
...
### Verification steps
```bash
jq -r '.subsystems.evaluation_diagnostics.files[]' docs/architecture/SUBSYSTEM_MAP.json
jq -r '.edges[] | select(.source_module=="evaluation" or .target_module=="evaluation") | .import_type' docs/audit/DEPENDENCY_EDGES.json | sort | uniq -c
```
